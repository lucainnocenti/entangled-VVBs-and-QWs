...
\begin{figure*}[ht]
    \centering
    \includegraphics[scale=0.5]{new_concept.pdf}
...
Suppose one round of entanglement transfer was executed successfully, so that the spaces $\calH^{(i)}_{\calW}$ are entangled, while the coins are separated due to the projections.
More precisely, this is a state of the form
\begin{equation}
	\sqrt2 \ket{\Psi} =
...
More precisely, we thus consider a state
\begin{equation}
	2 \ket{\Psi'} =
...
Suppose the output state after the first entanglement transfer stage had states
$\ket$
