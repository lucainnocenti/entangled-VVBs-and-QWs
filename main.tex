...
% Note that, if the projection probabilities $q_k$ are the same for all $k$, then $q_k=p_{\rm proj}$ and thus $\ket{\Psi_\gamma}$ displays the same entanglement as $\ket{\Psi}$:
% \begin{equation}
...
As discussed in~\cref{subsec:generalsolution_firstiteration_2Dcoin,subsec:analytical_results_QWs}, whereas the output of a single QW step always allows for optimal transferability, this is not always the case for the output of multiple steps.
We present in this section numerical results exploring the satisfiability of $\on{TC}_1$ for these states.
