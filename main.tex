...
Note that the task at hand can be broken down into two distinct entanglement transfer problems: restricting to \emph{local} projections on both sides, the transferability of entanglement between $\calH^{(1)}_{\calC}$ and $\calH^{(1)}_{\calW}$ is independent from that between $\calH^{(2)}_{\calC}$ and $\calH^{(2)}_{\calW}$.
Consequently, most of our discussion will focus, without loss of generality, on the transferability on a single side.

\parTitle{Condition for entanglement transfer}
Consider a state $\ket\Psi\in\calH$, and focus on the state of the first photon: $\calH^{(1)}$. As we only allow local operations in this setting, we have to deal with the reduced $\rho\equiv\Tr_2\ketbra\Psi$. Projecting onto a polarization state $\ket\gamma$ then gives $\rho_{\calW}\equiv \mel\gamma\rho\gamma\in\calH^{(1)}_{\calW}$.
We want the projection $\ket\gamma$ to be such that, \textit{in the full state}, the degree of entanglement is not degraded.
For this to be the case, it is necessary and sufficient that $\rho_{\calW}$ is \textit{diagonal in the same eigenbasis of $\rho$}.
...
% These have the form
% \begin{equation}
%     \ket*{\Psi_f} = \sum_k \sqrt{p_k} \ket{u_k}\ket{v_k},
%     \label{eq:general_psif}
% \end{equation}
% for some $p_k\ge0$ with $\sum_k p_k=1$ and orthonormal sets of vectors $\{\ket{u_k}\}_k\subset\calH_{AB}$ and $\{\ket{v_k}\}_k\in\calH_{CD}$.
...
% $\ket*{\tilde u_k}\equiv \braket*{\gamma_B}{u_k}$ and
% $\ket*{\tilde v_k}\equiv \braket*{\gamma_C}{v_k}$
% $\ket*{\tilde u_k},\ket*{\tilde v_k}$
% satisfy
...
\parTitle{Formal solution in general case}
The conditions $\braket*{\tilde u_j}{\tilde u_k}=0$ are equivalent to
\begin{equation}
    \mel{\gamma}{\tr_A(\ketbra*{u^j}{u^k})}{\gamma} = p^u_j \delta_{jk},
    \label{eq:orthogonality_condition_for_ent_transfer}
\end{equation}
and similarly for $\ket{v_k}$.
The task is therefore reduced to finding $\ket\gamma$ such that $\mel{\gamma}{C_{jk}}{\gamma}=0$, given $C_{jk}\equiv\tr_A(\ketbra*{u^j}{u^k})$ such that $\tr C_{jk}=\delta_{jk}$.
More generally, define the auxiliary state
$\ket*{\tilde\Psi_f}=\sum_k \sqrt{p_k}\ket*{u^k}\otimes\ket k$.
This state can be understood as a representation of $\ket{\Psi_f}$ in a suitable basis for $\calH_{CD}$.
The conditions for optimal entanglement transfer then amount to
$\mel{\gamma}{\tr_A(\tilde\Psi_f)}{\gamma}$ being diagonal.
Its diagonal elements are then the projection probabilities $p^u_j$.
% Optimal projection probability is achieved when $p^u_j=p^u$, and thus 
% $\mel{\gamma}{\tr_A(\tilde\Psi_f)}{\gamma}=p^u I$.
% The overall projection probability is then $p_{\on{proj}}=p^u p^v$.
Note that it is possible for more than one projection $\ket\gamma$ to achieve entanglement transfer. In such situations, one might accept multiple measurement results on $B,C$, thus further increasing the projection probability.
Some examples of this will be seen in the following sections.
% This probability can be upper-bounded in the general case as $p^u\le 1/N$ with $N$ the Schmidt rank of $\ket{\Psi_f}$. This follows from $\{\ket*{u^k}\}_k$ being an orthonormal set of $N$ vectors, and thus $1\ge \sum_k |\braket*{\gamma}{u^k}|^2=N p^u$.
% This upper-bounds the overall projection probability with $p_{\on{proj}}\le 1/N^2$.
% Nevertheless, different projections $\ket\gamma$ can result in optimal entanglement transfer, and possibly the same output state. In these cases, one can accept multiple outcomes, and there being $N$ possible orthogonal states in an $N$-dimensional system, achieve an improved projection probability of $p_{\on{proj}}\le 1/N$.
% Some examples of this type of situations will be given in the following section.
The projection probabilities equal $p^u_j=p^u=1$ when no entanglement transfer is taking place due to $A$ and $B$ being separated.
% Similarly, when $p^u$ is close to one, a partial trace over $B,C$ might be enough to obtain a sufficient amount of entanglement on the remaining degrees of freedom.
...
\subsection{\texorpdfstring{$2\to N$}{} entanglement transfer}

\parTitle{Problem setting}
While solving the problem in full generality is challenging, in the case of $\calH_B,\calH_C$ being two-dimensional we can derive a general solution for the projection $\ket\gamma$.
% We consider here the special case of $B,C$ being two-dimensional degrees of freedom.
For simplicity of notation, we will denote in this section the two possible states of the two-dimensional degree of freedom with ``$H$'' and ``$V$'', rather than ``$1$'' and ``$2$''.
The task is then to find $\ket\gamma$ such that
\begin{equation}
    \mel{\gamma}{\tr_A(\ketbra*{u^H}{u^V})}{\gamma} =
    \mel{\gamma}{\tr_A(\ketbra*{v^H}{v^V})}{\gamma} = 0.
\end{equation}
Let us assume for simplicity that $\ket*{v^V}=\ket*{u^H}$ and $\ket*{v^H}=\ket*{u^V}$.
This allows us to only have to deal with the condition over the $\ket*{u^s}$, and have the condition over the projection probabilities be automatically satisfied.

\parTitle{General solution for projection $\ket\gamma$}
Define the matrix $M\equiv \tr_A(\ketbra*{u^H}{u^V})$. The orthogonality relation $\braket*{u^H}{u^V}=0$ then implies $\tr M=0$.
Then, provided that $M$ is diagonalisable with $M\ket{\lambda_s}=\pm\lambda\ket{\lambda_s}$, define
\begin{equation}
...
\parTitle{Special case of normal $M$}
Interestingly, when $M$ is normal, there are always multiple viable projections.
Indeed, in this case we have $M=\sum_k \lambda_k \ketbra{\lambda_k}$ with $\sum_k\lambda_k=0$,
and thus given $\ket\gamma=\sum_k c_k\ket{\lambda_k}$ we have
$\mel{\gamma}{M}{\gamma}=\sum_k \lambda_k |c_k|^2$,
which vanishes whenever $|c_k|=1$.
We can then choose $\sqrt2\ket{\gamma_\pm}=\ket{\lambda_1}\pm\ket{\lambda_2}$, increasing the probability of entanglement transfer.
