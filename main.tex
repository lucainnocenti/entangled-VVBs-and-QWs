...
\highlight{(maybe move somewhere else)}
...
This is because any form of ``imperfect entanglement'' can be improved by means of local projections in some cases (\emph{i.e.} local projections are sufficient to transform a state of the form $\sum_k \sqrt{p_k}\ket{u_k}\ket{v_k}$ into the corresponding maximally entangled state $\sum_k \ket{u_k}\ket{v_k}$).
...
There are then two possibilities: either $s_1=s_2$, which implies $M$ is normal, and this case was covered above, or $s_1\neq s_2$, which implies by the uniqueness of the singular value decomposition that $\ket{w_1}=\ket2$ and $\ket{w_2}=\ket1$ up to phases.
...
We investigate here the residual entanglement when projecting the coin degree of freedom.
...
\begin{figure*}[t]
    ...
    \caption{\textbf{Projection optimization for Hadamard quantum walks.}
    a-b) State negativities for different types of projection $00$ and $01$ (the other two cases display the same behaviour) in terms of $(\theta, \phi)$. It is worth noting that $\mathcal{N}_{lm}>0.6$ in both cases.
    \highlight{(how many steps is this? how are the projection chosen ($00$ means same projection on both sides, but which one is being used)?)}
    c-d) Projection probabilities. It is possible to obtain $P_{lm}> 0.25$ (generation probability of~\cite{fickler2012quantum}) but with non-optimal negativity \highlight{(clarify sentence)}. e-f) $\mathcal{N}_{00}$ and $P_{00}$ respect with different number of steps in the case e) of $\mathcal{N}_{00}$ optimization and f) for the maximization of both $\mathcal{N}_{00}$ and $P_{00}$. We note that both bound ($1, 0.25$) can be saturated by our protocol. g-h) same calculation of panels e-f) for $01$ projection.
    \highlight{(what are $P^{Zeil}$ and $\calN^{MAX}$?)}
    }
    \label{fig:10steps_results}
\end{figure*}

\parTitle{Case with same QW on both sides}
Let us consider the case in which both parties evolve according to the same QW evolution.
The states in \cref{eq:prj_state_a} and  \cref{eq:prj_state_d} can be rewritten as $\alpha \ket{\psi_a \psi_b} + \beta \ket{\psi_b \psi_a}$, since only two type of qudits can be obtained.

\parTitle{Excursus on state generation (to move somewhere else probably)}
...
\highlight{(the message of this paragraph is not clear. Also, should this go in the overview or experimental sections?)}
...
\parTitle{Different unitaries} \highlight{We say above "\textit{We firstly investigate the case $U_1=U_2$}". Do we have data for different unitaries?}
...
While this is not generally the case, we can always formally describe the states this way, up to some relabelling.
...
\end{equation}
Clearly, this can only be satisfied for suitable choices of $\calU$ and $\ket{\psi_i}$. In particular, the dimension of $\calU$ must be such that even after the projection there are dimensions to accommodate the required number of orthogonal states.
...
The dimension of $\calU$ must therefore increase exponentially with the number of iterations, if one were to hope adding a new ebit of entanglement with each one.
...
\parTitle{Scaling of negativty}
Consider a state $\ket{q} = \sum_{s =1}^{n_q} \ket{ss}$, where $s$ are the walkers position. The negativity of such state scales as $\log_2(n_q + 1)$. According to such intuition we expect that the maximum negativity of the state $\ket{\Psi^{f(N_i)}}$ at the iteration $N_i$ after a proper coin projection, could be $\mathcal{N}^{Max} =\log_2( 2^{N_i}+1)$. Indeed the number of states in the superposition grows as $2^{N_i}$, as can be verified looking at~\eqref{eq:final_state} and~\eqref{eq:final_state1}.
...
In~\cref{fig:entanglement_accumulation_numerical_results}b we try to change the unitary after the first optimal coin projection on two Hadamard QWs \highlight{(clarify)}.
For the purpose, we sample random coin operators for the next QW stage. We find a dependence on both the choice of unitary in the second stage, and the number of steps of each QW (see~\cref{fig:entanglement_accumulation_numerical_results}b). The latter is not surprising, since with many steps we cover higher-dimensional Hilbert spaces and, consequently, it is easier to produce four mutually orthogonal qudits.
...
\begin{figure}[tb]
    ...
    a) State negativity for different iterations of the entanglement accumulation protocol. Both parties are here evolved with identical Hadamard QWs, for $N=\{1,3,5,7,10\}$ steps. From such simulation seems that the optimal transfer of one ebit per iteration cannot be reached. However, relaxing the need to have same unitaries at each step, the optimal scaling can be obtained. \highlight{(do we have data for this? doesn't the text say the opposite?)} b) Histograms for one iteration of entanglement transfer protocol (the stage zero with two identical Hadamard QWs), for $N=3,5, 7$ QW steps. 
    ...
    \highlight{(isn't this data for a single iteration? why are we talking about a second entanglement transfer?)}
    }
